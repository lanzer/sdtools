...
# Adapted from https://github.com/ototadana/sd-face-editor
class Face:
...
class Script(scripts.Script):
    def title(self):
        return "Dplace Face Editor (Testing)"
    ...
    def run(
        ...
        original_image_pil = Image.fromarray(entire_image)
        ...
        # Create mask for the entire image (black)
        ...
        for face in faces:
            ...
            # use prompt for the face, otherwise, use the original prompt for the img2img sample
            p.prompt = prompt_for_face if len(
                prompt_for_face) > 0 else entire_prompt
            p.do_not_save_samples = True

            # performe image to image in the face
            ...
            # DISPLAY new face image
            output_images.append(face_image)

            # DISPLAY masked face image
            x = self.__to_masked_image(mask_image, face_image)
            output_images.append(x)

            # resize to original face square shape
            ...
            # paste back new face square image in the original image
            entire_image[
            ...
            #  paste back only the face area in the original image
            t1 = Image.fromarray(entire_image)
            t2 = Image.fromarray(entire_mask_image)

            masking = t2.split()[0]
            original_image_pil.paste(t1, (0,0), mask=masking)

            entire_image = np.array(original_image_pil)
            entire_image = cv2.cvtColor(entire_image, cv2.COLOR_RGB2RGBA)

            
        # DISPLAY original image with the new face overlapped
        output_images.append(entire_image)

        # DISPLAY masked image
        y = self.__to_masked_image(entire_mask_image, entire_image)
    ...
    def __crop(self, image:Image, face_boxes, face_margin:float):
